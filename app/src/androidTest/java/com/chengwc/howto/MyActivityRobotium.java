...
public class MyActivityRobotium extends ActivityInstrumentationTestCase2<MyActivity> {
  ...
  public void testMyActivityWithRobotium() {
    ...
    Assert.assertTrue(solo.searchText(getActivity().getString(com.chengwc.howto.R.string.hello_world)));

    // Click a button which will start a new Activity
    // Here we use the ID of the string to find the right button
    solo.clickOnButton(solo.getString(com.chengwc.howto.R.string.click_me));
    Assert.assertTrue(solo.waitForText(getActivity().getString(com.chengwc.howto.R.string.ok_thanks)));
